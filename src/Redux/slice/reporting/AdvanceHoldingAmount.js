...
export const pmaSlice = createSlice({
  ...
  reducers: {
    ...
    setAdvanceHoldingAmountFilters: (state, { payload }) => {
...
export const {
  ...
  setPageNumber,
  setCountPerPage,
  setAdvanceHoldingAmountFilters,
  setInitialState,
  setSorting,
} = pmaSlice.actions;
  ...
  (payloadObj, year, month) => async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${env_URL_SERVER}reportAdvanceHoldingAmount`,
        payloadObj
      );
  ...
  export const downloadXlsEndpoint = (filename, userId ,type='excel') => async (dispatch) => {
    try {
      ...
      if(type == 'excel') {
        ...
        FileSaver.saveAs(blob, "AdvanceHoldingAmount.pdf");
      }
