...
export const downloadOrderPaymentB2BReport = (payloadObj ,type) => async (
  dispatch
) => {
  try {
    dispatch(setStatus("loading"));
...
export const downloadXlsEndpoint = (filename, userId ,type='excel') => async (dispatch) => {
  try {
    ...
    if(type == 'excel') {
      FileSaver.saveAs(blob, "Order-payment-B2B.xlsx");
    }else {
      FileSaver.saveAs(blob, "Order-payment-B2B.pdf");
    }
  } catch (error) {
