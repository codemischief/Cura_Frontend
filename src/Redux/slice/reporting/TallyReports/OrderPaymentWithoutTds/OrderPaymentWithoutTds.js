...
export const orderPaymentWithoutTds = createSlice({
  ...
  reducers: {
    setOrderPaymentWithoutTdsView: (state, { payload }) => {
      const { data } = payload;
      state.orderPaymentWithoutTdsView = clientReceiptFormatData(data.data)
...
export const downloadOrderPaymentWithoutTdsReport = (payloadObj ,type) => async (
  dispatch
) => {
  try {
    dispatch(setStatus("loading"));
...
export const downloadXlsEndpoint = (filename, userId ,type='excel') => async (dispatch) => {
  try {
    ...
    if(type == 'excel') {
      ...
      FileSaver.saveAs(blob, "Order-payment-without-TDS.pdf");
    }
  } catch (error) {
