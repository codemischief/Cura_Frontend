...
export const PropertyWithNoProjectSlice = createSlice({
  name: "PropertyWithNoProject",
  initialState,
  reducers: {
    setPropertyWithNoProject: (state, { payload }) => {
      const { data } = payload.data;
      state.propertyWithNoProject = (data);
    ...
    setPropertyWithNoProjectFilter: (state, { payload }) => {
...
export const getPropertyWithNoProject = (payloadObj) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      `${env_URL_SERVER}reportExceptionPropertiesNoProjects`,
      payloadObj
    );

    dispatch(setPropertyWithNoProject({ data: response.data }));
...
export default PropertyWithNoProjectSlice.reducer;