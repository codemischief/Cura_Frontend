...
export const pmaSlice = createSlice({
  ...
  initialState,
  reducers: {
    setOrderReceiptData: (state, { payload }) => {
      const { data, year, month } = payload;
      state.orderReceiptData = updatedOrderReceiptData(data.data, year, month);
    ...
    setOrderReceiptFilters: (state, { payload }) => {
...
export const {
  ...
  setPageNumber,
  setCountPerPage,
  setOrderReceiptFilters,
  setInitialState,
  setSorting,
  resetData
} = pmaSlice.actions;

export const getOrderReceiptData =
  (payloadObj, year, month) => async (dispatch) => {
    console.log("called");
    try {
      dispatch(setStatus("loading"));
      const response = await axios.post(
        `${env_URL_SERVER}getReportOrderReceipt`,
        payloadObj
      );

      dispatch(setOrderReceiptData({ data: response.data, year, month }));
  ...
  export const downloadXlsEndpoint = (filename, userId , type='excel') => async (dispatch) => {
    try {
      ...
      if(type == 'excel') {
        ...
        FileSaver.saveAs(blob, "OrderReceiptList.pdf");
      }
