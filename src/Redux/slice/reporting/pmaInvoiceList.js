...
export const pmaSlice = createSlice({
  ...
  reducers: {
    setPmaInvoiceList: (state, { payload }) => {
      ...
      state.pmaInvoiceList = pmaInvoiceList(data.data, year, month);
...
export const {
  ...
  setPageNumber,
  setCountPerPage,
  setPmaInvoiceListFilters,
  setInitialState,
  setSorting,
  resetFilters
} = pmaSlice.actions;

export const getPmaInvoiceList =
  (payloadObj, year, month) => async (dispatch) => {
    try {
      dispatch(setStatus("loading"));
      const response = await axios.post(
        `${env_URL_SERVER}reportPMABillingListView`,
        payloadObj
      );
        
      dispatch(setPmaInvoiceList({ data: response.data, year, month }));
...
export const downloadPmaInvoiceList =
  ...
  export const downloadXlsEndpoint = (filename, userId ,type='excel') => async (dispatch) => {
    try {
      ...
      if(type == 'excel') {
        ...
        FileSaver.saveAs(blob, "ReportPmaInvoiceList.pdf");
      }
    } catch (error) {
