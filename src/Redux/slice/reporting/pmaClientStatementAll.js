...
export const pmaSlice = createSlice({
  name: "pmaClientStatementAll",
  initialState,
  reducers: {
    setPmaClientStatementAll: (state, { payload }) => {
      const { data, year, month } = payload;
      state.pmaClientStatementAll = pmaClientStatementAll(data.data, year, month);
    ...
    setPmaClientStatementAllFilters: (state, { payload }) => {
...
export const {
  setPmaClientStatementAll,
  setStatus,
  setPageNumber,
  setCountPerPage,
  setPmaClientStatementAllFilters,
  setInitialState,
  setSorting,
} = pmaSlice.actions;

export const getPmaClientStatementAll =
  (payloadObj, year, month) => async (dispatch) => {
    try {
      dispatch(setStatus("loading"));
      const response = await axios.post(
        `${env_URL_SERVER}reportPMAClientAll`,
        payloadObj
      );

      dispatch(setPmaClientStatementAll({ data: response.data, year, month }));
