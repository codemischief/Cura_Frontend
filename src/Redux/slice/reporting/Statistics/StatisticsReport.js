...
export const pmaSlice = createSlice({
  ...
  reducers: {
    setData: (state, { payload }) => {
      ...
      state.data = StatisticsReport(data.data, year, month);
  ...
  (payloadObj, year, month) => async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${env_URL_SERVER}reportStatisticsReport`,
        payloadObj
      );
...
export const downloadDataXls =
  ...
  export const downloadXlsEndpoint = (filename, userId ,type='excel') => async (dispatch) => {
    try {
      ...
      if(type == 'excel') {
        FileSaver.saveAs(blob, "StatisticsReport.xlsx");
      }else {
        FileSaver.saveAs(blob, "StatisticsReport.pdf");
      }
    } catch (error) {
