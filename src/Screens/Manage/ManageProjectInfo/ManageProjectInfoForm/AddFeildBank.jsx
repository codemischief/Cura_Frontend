...
const AddFeildBank = ({ index, formValues, setFormValues, accType }) => {
  ...
  return (
    <div className="w-full h-[40px] flex border-[#CBCBCB] border-b-[1px]">
      ...
      <div className="w-[12%] h-full py-1 px-3 text-[11px]" >
        <select className='w-full h-full bg-[#F5F5F5] px-1 outline-none' type="text" value={formValues.project_bank_details[index].banktypeofaccount} name="banktypeofaccount" onChange={handleChange}>
        <option value="none" hidden></option>
          {accType && accType.map(item => (
