...
const EditClientProperty = (props) => {
  ...
  const fetchExistingData = async () => {
    ...
    setFormValues({
      ...
      client_property_owner: {
        ...deepCopyData?.client_property_owner,
        propertyid: Number(deepCopyData?.client_property?.id),
      },
    });
  ...
  function helper1(
    updateArrayClientPhotos,
    insertArrayClientPhotos,
    deleteArrayClientPhotos
  ) {
    var size = formValues.client_property_photos.length;
    const initialPhotos = JSON.parse(
      JSON.stringify(intialProperty.client_property_photos)
    );
    for (var i = 0; i < size; i++) {
      ...
      if (tempObj.hasOwnProperty("id")) {
        ...
        Object.keys(formValues.client_property_photos[i]).forEach((key) => {
          if (
            formValues.client_property_photos[i][key] != null &&
            formValues.client_property_photos[i][key] != ""
          ) {
            flag = true;
          }
        });
  ...
  const handleEdit = async () => {
    ...
    const data = {
      ...
      client_property_info: {
        ...
        propertytype: formValues.client_property.propertytype,
        leveloffurnishing: formValues.client_property.leveloffurnishing,
        numberofparkings: formValues.client_property.numberofparkings,
        ...
        status: formValues.client_property.status,
        ...
        electricityconsumernumber:
          formValues.client_property.electricityconsumernumber,
        otherelectricitydetails:
          formValues.client_property.otherelectricitydetails,
        electricitybillingduedate:
          formValues.client_property.electricitybillingduedate,
        ...
        propertyownedbyclientonly:
          formValues.client_property.propertyownedbyclientonly,
      },
      client_property_photos: {
        ...
        <Draggable handle="div.move">
          <div className="w-[1150px] h-auto bg-white rounded-lg">
              ...
              <div className="h-[40px] bg-[#EDF3FF]  justify-center flex items-center rounded-lg">
                <div className="mr-[410px] ml-[410px]">
                  <div className="text-[16px]">
                    Edit Client Property :{intialProperty?.client_property?.id}{" "}
                  </div>
                </div>
            ...
            <div className="mt-1 flex bg-[#DAE7FF] justify-center space-x-4 items-center h-9">
              <div
                className={`${
                  selectedDialog == 1 ? "bg-blue-200" : "bg-[#EBEBEB]"
                } px-4 py-1 rounded-md text-[12px] font-semibold flex justify-center items-center h-7 w-60 cursor-pointer`}
              ...
              <div
                className={`${
                  selectedDialog == 2 ? "bg-blue-200" : "bg-[#EBEBEB]"
                } px-4 py-1 rounded-md text-[12px] font-semibold flex justify-center items-center h-7 w-60 cursor-pointer`}
                onClick={() => setSelectedDialogue(2)}
              >
                <div>Photos</div>
              </div>
              <div
                className={`${
                  selectedDialog == 3 ? "bg-blue-200" : "bg-[#EBEBEB]"
                } px-4 py-1 rounded-md text-[12px] font-semibold flex justify-center items-center h-7 w-60 cursor-pointer`}
                onClick={() => setSelectedDialogue(3)}
              >
                <div>POA Details</div>
              </div>
              <div
                className={`${
                  selectedDialog == 4 ? "bg-blue-200" : "bg-[#EBEBEB]"
                } px-4 py-1 rounded-md text-[12px] font-semibold flex justify-center items-center h-7 w-60 cursor-pointer`}
                onClick={() => setSelectedDialogue(4)}
              >
            ...
            {!pageLoading && selectedDialog == 1 && (
              <EditProjectInformation
                clientData={clientData}
                initialCountries={allCountry}
                initialSociety={existingSociety}
                initialStates={allState}
                initialCities={allCity}
                clientTypeData={clientTypeData}
                formValues={formValues}
                setFormValues={setFormValues}
                propertyType={propertyType}
                levelOfFurnishing={levelOfFurnishing}
            ...
            <div className="my-2 flex justify-center items-center gap-[10px]">
              <button
                className="w-[100px] h-[35px] bg-[#004DD7] text-
white rounded-md"
                onClick={handleEdit}
              >
                Save
