...
import { setReceiptUnderSuspenseOrderFilter,setPageNumber } from "../../../../Redux/slice/reporting/Group12/ReceiptsUnderSuspenseOrderSlice";

export function CustomFilterField(props) {
  const dispatch = useDispatch();
  const { filter } = useSelector((state) => state.receiptsUnderSuspenseOrder);

  const handleFilterChange = (filters) => {
    dispatch(setReceiptUnderSuspenseOrderFilter(filters));
