...
const NumericFilter = ({inputVariable,setInputVariable,handleFilter,columnName,menuRef}) => {
  return (
    <div className='h-[360px] w-[150px] mt-3 bg-white shadow-xl font-thin font-sans absolute p-2 flex-col rounded-md space-y-1 text-sm z-40' ref={menuRef}>
                                    ...
                                    <div className='hover:bg-[#dae7ff] p-1 rounded-sm cursor-pointer'>
                                        <button onClick={() => handleFilter(inputVariable,setInputVariable,'between', columnName)}><h1 >Between</h1></button>
                                    </div>
                                    <div className='hover:bg-[#dae7ff] p-1 rounded-sm cursor-pointer'>
                                        <button onClick={() => handleFilter(inputVariable,setInputVariable,'notBetween', columnName)}><h1 >NotBetween</h1></button>
                                    </div>
                                    <div className='hover:bg-[#dae7ff] p-1 rounded-sm cursor-pointer'>
                                        <button onClick={() => handleFilter(inputVariable,setInputVariable,'isNull', columnName)}><h1 >isNull</h1></button>
                                    </div>
                                    <div className='hover:bg-[#dae7ff] p-1 rounded-sm cursor-pointer'>
                                        <button onClick={() => handleFilter(inputVariable,setInputVariable,'isNotNull', columnName)}><h1 >isNotNull</h1></button>
                                    </div>
                                </div>
  )
}
